...
class QuantizeLinear(nn.Linear):
    def __init__(
        ...
        *kargs,
        ...
        self.weight_noise_scale = None
        self.clip_noise_scale = None
    ...
    def _apply_noise_config(self, config):
        self.noise_injection = getattr(config, "noise_injection", False)
        self.pre_quantization_noise = getattr(config, "pre_quantization_noise", False)
        self.post_quantization_noise = getattr(config, "post_quantization_noise", False)
        self.initialize_noise = getattr(config, "initialize_noise", False)
        self.noise_sigma_weights = float(getattr(config, "noise_sigma_weights", 0.0))
        self.noise_sigma_clipvals = float(getattr(config, "noise_sigma_clipvals", 0.0))
        self.trainable_noise_scale = getattr(config, "trainable_noise_scale", False)
    ...
    def quantized_weight(self, dtype=None, apply_noise=False):
        ...
        if (
            ...
            and apply_noise
            and self.training
            and self.post_quantization_noise
        ):
